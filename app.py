...
def venues():
  ...
  states = Venue.query.with_entities(Venue.state, Venue.city).group_by(Venue.state, Venue.city).all()
  data = []
  for state, city in states:
    venues = Venue.query.with_entities(Venue.id, Venue.name).filter_by(state=state, city=city).order_by('id').all()
